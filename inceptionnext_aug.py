...
class CustomDataset(Dataset):
    def __init__(self, image_paths, labels, mask_paths, transform=None):
        self.image_paths = image_paths
        self.mask_paths = mask_paths  # Corrected the order of parameters
        self.labels = labels
    ...
    def overlay_mask(self, image, mask):
        overlaid_image = np.where(mask == 255, image, np.clip(mask.astype(int) + 50, 0, 255)).astype(np.uint8)
        return Image.fromarray(overlaid_image)

    def __getitem__(self, idx):
        image = Image.open(self.image_paths[idx])
        mask_path = self.mask_paths[idx]  # Retrieve the mask path
        mask = Image.open(mask_path)  # Open the mask using the correct path

        # Apply overlay operation
        overlaid_image = self.overlay_mask(np.array(image), np.array(mask))

        if self.transform:
            overlaid_image = self.transform(overlaid_image)

        label = self.labels[idx]
        return overlaid_image, label
...
imageData=pd.read_csv('data/stage_1_train_images.csv').reset_index(drop=True)
...
for i in range(len(X)):
    file_name = 'data/png_images/' + X[i]
    image_paths.append(file_name)

mask_paths = []

for i in range(len(X)):
    file_name = 'data/png_masks/' + X[i]
...
for fold, (train_index, val_index) in enumerate(kf.split(image_paths)):
    ...
    custom_dataset = CustomDataset(image_paths, labels, mask_paths, transform=transform)
...
imageData=pd.read_csv('data/stage_1_test_images.csv').reset_index(drop=True)
...
for i in range(len(imageData)):
    file_name = 'data/png_images/' + imageData['new_filename'][i]
    image_paths.append(file_name)

mask_paths = []

for i in range(len(imageData)):
    file_name = 'data/png_masks/' + imageData['new_filename'][i]
    mask_paths.append(file_name)
...
specifier = "InceptionNeXt"

fpr, tpr, _ = skl.metrics.roc_curve(all_labels, all_predicted)
print("\n%s ROC AUC Score: %.4f\n" % (specifier, skl.metrics.roc_auc_score(all_labels, all_predicted)))
plt.plot(fpr, tpr)
plt.ylabel("True Positive Rate")
plt.xlabel("False Positive Rate")
plt.savefig("with_masks_roc.png")
